module AresMUSH
  module Pf2e
    ...
    # from an earlier encounter carries that one's copy on. When the encounter ends, only consumables go back
    # to their own inventory: what they used up comes off it, and what the encounter gave them goes on.
    module Equipment
      ...
      # The encounter has ended: what it did to the character's consumables happens to their own. Answers
      # what it could not do - an item they no longer have - as events, for whoever tells it.
      def self.settle!(state)
        ...
        now.reject(&:copied_from).each do |gained|
          own = gained.attributes.except(:character_id, :state_id, :created_at, :updated_at, :copied_from, :bag_id)
          gained.update(:copied_from => PF2Consumable.create(own.merge(:character => char)).id)
        end
